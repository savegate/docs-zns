...
|---|---|
| Official Account đã kết nối | Console → Official Account |
| Template ở trạng thái `ENABLE` | Console → Template |
| Ví workspace còn số dư | Console → Tổng quan |
| API key | Console → Cài đặt → API Keys |

<Warning>
API vận hành ở chế độ production. Không có môi trường sandbox. Mỗi request thành
công tạo một tin thật và trừ phí từ ví workspace theo giá template.

Để kiểm thử nội dung template không mất phí, dùng chức năng **Gửi thử** trong
Console → Template. Chức năng này chỉ gửi tới số quản trị viên của OA.
</Warning>

## Luồng tích hợp

<CardGroup cols={2}>
  <Card title="1. Lấy template" icon="list">
